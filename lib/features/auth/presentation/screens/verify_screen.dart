...
import 'package:flutter_ursffiver/features/auth/controller/forget_password controller.dart';
...
class _VerifyScreenState extends State<VerifyScreen> {
  ...
  final _nodes = List.generate(6, (_) => FocusNode(), growable: false);
  final _controllers = List.generate(6, (_) => TextEditingController(), growable: false);
  ...
  void initState() {
    ...
    resendController = ForgetPasswordController(
      snackbarNotifier: SnackbarNotifier(context: context),
    );
    resendController.email = widget.email;

    // Only start timer when it's NOT from register screen
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'We have shared a code to your email\n${widget.email}',
                  style: caption,
                ...
                ///
                /// 🔥 RESEND OTP (visible only for forget password flow)
                ///
                if (!widget.isFromRegisterScreen)
                  Center(
                    child: _secondsLeft > 0
                        ? Text(
                            "Resend available in ${_formatTime(_secondsLeft)}",
                            style: caption,
                          )
                        : GestureDetector(
                            onTap: () async {
                              await resendController.forgetPassword(
                                buttonNotifier: resendController.processNotifier,
                                snackbarNotifier: SnackbarNotifier(context: context),
                              );
                              _startTimer();
                            },
